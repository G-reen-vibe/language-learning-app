...
interface SentenceQ {
  ...
  /** index into parsed tokens for each blank */
  blankTokenIndices: number[];
  /** the correct word for each blank (in blank order) */
  correctWords: string[];
  /** wordKey for each blank (for grading) */
  wordKeys: (string | null)[];
  /** pieces: correct words + distractors, shuffled */
  pieces: string[];
  /** the rendered sentence tokens (with blank info) */
...
export default function SentenceComprehensionFormat({
  lesson,
  eligibleWords,
  onResult,
  onDone,
  ...
  const questions = useMemo<SentenceQ[]>(() => {
    ...
    while (out.length < numQuestions && attempts < numQuestions * 8 && suitableEligible.length > 0) {
      ...
      for (let i = 0; i < parsed.tokens.length; i++) {
        ...
        if (!t.translation) continue; // only words with translations are real "word" tokens
        if (!t.text.trim()) continue;
        // strip punctuation from text for matching
      ...
      // nBlanks: 2-4, scaling with mastery on the continuous [0,1] scale.
      //   mastery < 0.25 → 2 blanks
      //   mastery < 0.50 → 3 blanks
      //   mastery < 0.75 → 3 blanks
      //   mastery >= 0.75 → 4 blanks
      // (Capped at 4 — matches the original cap.)
      const nBlanks = Math.min(
        Math.max(2, blankCandidates.length),
        mastery >= 0.75 ? 4 : mastery >= 0.25 ? 3 : 2
      );
      ...
      // correct words for blanks (in token order)
      const blankTokenIndices = chosen.map((c) => c.idx).sort((a, b) => a - b);
      const correctWords = blankTokenIndices.map((idx) => {
      ...
      const tokens = parsed.tokens.map((t, i) => {
        ...
        // determine if this token's word has mastery >= 3 (translation hidden)
  ...
  const [placed, setPlaced] = useState<(number | null)[]>([]); // piece indices for each blank
  ...
  const handleBlankClick = useCallback(
    (blankIdx: number) => {
      if (feedback !== "none") return;
      // remove the piece from this blank
      setPlaced((prev) => {
        const next = [...prev];
        next[blankIdx] = null;
        return next;
  ...
  const handleSubmit = useCallback(() => {
    ...
    // Grade each blank as a separate result
  ...
  const handleNext = useCallback(() => {
    if (idx + 1 >= questions.length) {
      if (!completed) {
        setCompleted(true);
        // results may have multiple per question (one per blank); questionCount = questions.length
  ...
  // Render the sentence with blanks
  let blankCounter = 0;
  return (
      ...
      <Card className="w-full">
        <CardHeader className="pb-3">
          <CardTitle className="text-center text-sm text-muted-foreground">
            Question {idx + 1} of {questions.length} — Sentence Comprehension
          </CardTitle>
        </CardHeader>
